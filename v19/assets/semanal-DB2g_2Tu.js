import{j as e}from"./vendor-gzd0YkcT.js";import{db as O}from"./firebase-Lfd52dYI.js";import{g as ss,q as as,c as es,w as os,s as is,h as ts,f as H,i as ls}from"./firebase-Cdwg_wrl.js";import{i as cs,c as h,o as y,g as q,s as ns,p as G,h as $,m as L}from"./main-CuczXLwU.js";const V="wii_horario_v1",T="horario",ds=["Lunes","Martes","Miércoles","Jueves","Viernes","Sábado","Domingo"],v={trabajo:{label:"Trabajo",icon:"fa-briefcase",color:"#0EBEFF"},estudio:{label:"Estudio",icon:"fa-book",color:"#7000FF"},personal:{label:"Personal",icon:"fa-user",color:"#FFB800"},salud:{label:"Salud",icon:"fa-heart-pulse",color:"#FF5C69"},proyecto:{label:"Proyecto",icon:"fa-diagram-project",color:"#A855F7"},reunion:{label:"Reunión",icon:"fa-users",color:"#29C72E"},otro:{label:"Otro",icon:"fa-circle",color:"#94A3B8"}},C={alta:{label:"Alta",color:"#FF5C69",icon:"fa-arrow-up"},media:{label:"Media",color:"#FFB800",icon:"fa-minus"},baja:{label:"Baja",color:"#29C72E",icon:"fa-arrow-down"}},x=["#0EBEFF","#7000FF","#FFB800","#FF5C69","#29C72E","#A855F7","#00C9B1","#94A3B8"],P=()=>new Date().toISOString().split("T")[0],D=(a=new Date)=>{const s=new Date(a),o=s.getDay()||7;return s.setDate(s.getDate()-o+1),s.toISOString().split("T")[0]},f=(a,s)=>{const o=new Date(a+"T12:00:00");return o.setDate(o.getDate()+s),o.toISOString().split("T")[0]},A=a=>new Date(a+"T00:00:00").toLocaleDateString("es-PE",{day:"numeric",month:"short"}),rs=a=>new Date(a+"T00:00:00").toLocaleDateString("es-PE",{weekday:"long",day:"numeric",month:"long"}),_s=a=>`${(a||"hor").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/\s+/g,"_").replace(/[^a-z0-9_]/g,"").slice(0,25)}_${Date.now()}`,ms=a=>a===P(),fs=a=>a<P(),N=a=>{const s=a.length,o=a.filter(t=>t.estado==="completado").length;return s?Math.round(o/s*100):0},B=()=>q("wiSmile")||null,M=()=>!!B()?.usuario,m=()=>q(V)||[],j=a=>ns(V,a,48),n=a=>{const s=e("#sem_sync");s.length&&(s[0].className=`sem_sync_dot sem_sync_${a}`)},R=async(a=!1)=>{if(!a&&m().length)return n("ok");if(!M())return n("error");n("loading");try{const s=await ss(as(es(O,T),os("usuario","==",B().usuario)));j(s.docs.map(o=>({...o.data(),_fsId:o.id,id:o.id}))),n("ok")}catch(s){console.error("❌ semanal:",s),n("error")}},U=a=>{const s=m(),o=a._fsId||_s(a.titulo),t={...a,_fsId:o,id:o},k=s.findIndex(p=>p._fsId===o);if(k>=0?s.splice(k,1,t):s.push(t),j(s),M()){n("saving");const p=B()||{},u={...t};delete u._fsId,is(H(O,T,o),{...u,usuario:p.usuario||"",email:p.email||"",actualizado:ts()},{merge:!0}).then(()=>n("ok")).catch(I=>{console.error("❌ upsert:",I),n("error")})}return t},vs=a=>{const s=a._fsId||a.id;j(m().filter(o=>o._fsId!==s)),M()&&(n("saving"),ls(H(O,T,s)).then(()=>n("ok")).catch(o=>{console.error("❌ del:",o),n("error")}))};let l=D(),d=null,X=null;const ps=()=>{const a=f(l,6);return m().filter(s=>s.fecha>=l&&s.fecha<=a)},us=a=>m().filter(s=>s.fecha===a),Ds=()=>`
<div class="sem_wrap">
