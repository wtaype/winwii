...
<div class="wiModal" id="modal_mes_confirm">
  <div class="modalBody mes_modal_confirm">
...
</div>`,$=()=>{const s=a("#mes_grid").empty(),e=gs(f,n),o=hs(f,n),t=Es(),c=d();a("#mes_titulo_mes").text(`${fs[n]} ${f}`);const _=t.filter(i=>i.done).length,v=t.filter(i=>i.prioridad==="alta").length,y=t.length?Math.round(_/t.length*100):0;a("#mes_n_total").text(t.length),a("#mes_n_done").text(_),a("#mes_n_alta").text(v),a("#mes_prog_label").text(`${_} / ${t.length}`),a("#mes_prog_fill").css("width",`${y}%`),a("#mes_prog_pct").text(`${y}%`);for(let i=0;i<e;i++)s.append('<div class="mes_cell mes_cell_vacio"></div>');for(let i=1;i<=o;i++){const h=$s(f,n,i),F=W(h),z=h===c,ss=h<c,es=h===r,H=new Date(h+"T00:00:00").getDay(),as=H===0||H===6,os=F.slice(0,3).map(C=>`<span class="mes_dot" style="background:${C.color||(b[C.categoria]||b.otro).color}" title="${C.titulo}"></span>`).join(""),ts=F.length>3?`<span class="mes_dot_mas">+${F.length-3}</span>`:"";s.append(`
    ...
    </div>`)})},X=s=>{a("#m_colores .mes_color_opt").removeClass("active"),a(`#m_colores .mes_color_opt[data-color="${s}"]`).addClass("active")},Z=()=>a("#m_colores .mes_color_opt.active").data("color")||B[0],M=(s,e)=>{a("#mes_m_hero").css("background",`linear-gradient(135deg,${s}dd,${s}88)`),a("#mes_m_ico").css("background",s).html(`<i class="fas ${e}"></i>`)},Ds=()=>{const s=r||d(),e=a("#m_fecha").val()||s;N(s,e)},k=(s={},e=null)=>{u=s._fsId?s:null;const o=s.fecha||e||r||d(),t=s.categoria||"trabajo",c=s.color||b[t]?.color||B[0];a("#m_titulo").val(s.titulo||""),a("#m_nota").val(s.nota||""),a("#m_fecha").val(o),a("#m_hora").val(s.hora||""),a("#m_cat").val(t),a("#m_prio").val(s.prioridad||"media"),X(c),M(c,b[t]?.icon||"fa-calendar-plus"),a("#mes_m_tit").text(u?"Editar Evento":"Nuevo Evento"),a("#mes_m_sub").text(u?"Modifica los datos":`Para: ${T(o)}`),a("#m_eliminar").toggleClass("dpn",!u),N(o,o),J("modal_mes"),setTimeout(()=>a("#m_titulo").focus(),30)},q=()=>{const s=a("#m_titulo").val().trim();if(!s)return w("Título requerido","warning");const e=a("#m_fecha").val();if(!e)return w("Fecha requerida","warning");S("#m_guardar",!0,"Guardar");const o=a("#m_cat").val();K({...u||{},titulo:s,nota:a("#m_nota").val().trim(),fecha:e,hora:a("#m_hora").val()||"",categoria:o,prioridad:a("#m_prio").val(),color:Z(),done:u?.done||!1,creado:u?.creado||new Date().toISOString()}),D("modal_mes");const[t,c]=e.split("-").map(Number);(t!==f||c-1!==n)&&(f=t,n=c-1),r=e,$(),setTimeout(()=>a(`.mes_cell[data-fecha="${e}"]`).addClass("mes_cell_sel"),30),S("#m_guardar",!1,"Guardar"),w(u?"Evento actualizado ✓":"Evento creado ✓","success")},Fs=s=>{const e=p().find(o=>o._fsId===s);e&&(K({...e,done:!e.done}),$(),e.done||w("✅ ¡Evento completado!","success"))},G=s=>{a("#mes_confirm_nombre").text(s.titulo||"Sin título"),Q=()=>{ys(s),D("modal_mes_confirm"),$(),w("Evento eliminado ✓","success")},J("modal_mes_confirm")},ks=()=>{a(document).off(".mes"),a(document).on("click.mes","#mes_prev",()=>{n===0?(n=11,f--):n--,$()}).on("click.mes","#mes_next",()=>{n===11?(n=0,f++):n++,$()}).on("click.mes","#mes_hoy",()=>{f=new Date().getFullYear(),n=new Date().getMonth(),r=d(),$()}).on("click.mes","#mes_refresh",async()=>{S("#mes_refresh",!0,""),localStorage.removeItem(x),await V(!0),$(),S("#mes_refresh",!1,""),w("Mes actualizado ✓","success")}).on("click.mes",".mes_cell:not(.mes_cell_vacio)",function(){a(".mes_cell").removeClass("mes_cell_sel"),a(this).addClass("mes_cell_sel"),R(a(this).data("fecha"))}).on("click.mes","#mes_nuevo",()=>k({},r||d())).on("click.mes","#mes_detail_add",function(){k({},a(this).data("fecha")||r||d())}).on("click.mes",".mes_ev_check",function(e){e.stopPropagation(),Fs(a(this).data("id"))}).on("click.mes",".mes_ev_item",function(e){if(a(e.target).closest(".mes_ev_check,.mes_ev_edit,.mes_ev_del").length)return;const o=p().find(t=>t._fsId===a(this).data("id"));o&&k(o)}).on("click.mes",".mes_ev_edit",function(e){e.stopPropagation();const o=p().find(t=>t._fsId===a(this).data("id"));o&&k(o)}).on("click.mes",".mes_ev_del",function(e){e.stopPropagation();const o=p().find(t=>t._fsId===a(this).data("id"));o&&G(o)}).on("click.mes",".mes_sug_btn",function(){const e=a(this).data("fecha");a("#m_fecha").val(e);const o=r||d();N(o,e),a("#mes_m_sub").text(`Para: ${T(e)}`)}).on("change.mes","#m_fecha",Ds).on("change.mes","#m_cat",function(){M(Z(),b[a(this).val()]?.icon||"fa-calendar-plus")}).on("click.mes","#m_colores .mes_color_opt",function(){X(a(this).data("color")),M(a(this).data("color"),b[a("#m_cat").val()]?.icon||"fa-calendar-plus")}).on("click.mes","#m_cancelar",()=>D("modal_mes")).on("click.mes","#m_guardar",q).on("keydown.mes","#m_titulo",e=>{e.key==="Enter"&&q()}).on("click.mes","#m_eliminar",()=>{u&&(D("modal_mes"),G(u))}).on("click.mes","#mes_conf_no",()=>D("modal_mes_confirm")).on("click.mes","#mes_conf_si",()=>Q?.())},Os=async()=>{f=new Date().getFullYear(),n=new Date().getMonth(),await V(),$(),r=d(),R(d()),setTimeout(()=>a(`.mes_cell[data-fecha="${d()}"]`).addClass("mes_cell_sel"),50),ks(),console.log("📆 Mes v1.0 OK")},Bs=()=>{a(document).off(".mes"),console.log("🧹 Mes limpiado")};export{Bs as cleanup,Os as init,xs as render};
