import{j as d}from"./vendor-gzd0YkcT.js";import{db as I}from"./firebase-BS8Opw9z.js";import{g as H,q as L,c as z,w as R,s as Q,f as A,i as G,h as U}from"./firebase-CiLZDPlJ.js";import{c as t,q as p,j as m,t as M,g as O,s as X}from"./main-BEFDoIKE.js";const B="wii_horario_v1",F="horario",Y=Array.from({length:24},(a,s)=>s),v={trabajo:{label:"Trabajo",icon:"fa-briefcase",color:"#0EBEFF"},estudio:{label:"Estudio",icon:"fa-book",color:"#7000FF"},personal:{label:"Personal",icon:"fa-user",color:"#FFB800"},salud:{label:"Salud",icon:"fa-heart-pulse",color:"#FF5C69"},proyecto:{label:"Proyecto",icon:"fa-diagram-project",color:"#A855F7"},reunion:{label:"Reunión",icon:"fa-users",color:"#29C72E"},otro:{label:"Otro",icon:"fa-circle",color:"#94A3B8"}},T={alta:"#FF5C69",media:"#FFB800",baja:"#29C72E"},D=["#0EBEFF","#7000FF","#FFB800","#FF5C69","#29C72E","#A855F7","#00C9B1","#94A3B8"];let e=new Date().toISOString().split("T")[0],n=null;const k=()=>new Date().toISOString().split("T")[0],J=a=>new Date(a+"T00:00:00").toLocaleDateString("es-PE",{weekday:"long",day:"numeric",month:"long",year:"numeric"}),b=(a,s)=>{const i=new Date(a+"T12:00:00");return i.setDate(i.getDate()+s),i.toISOString().split("T")[0]},w=a=>a===k(),K=a=>a<k(),V=a=>`${(a||"hor").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/\s+/g,"_").replace(/[^a-z0-9_]/g,"").slice(0,25)}_${Date.now()}`,E=a=>a/60*6,u=a=>{const[s,i]=a.split(":").map(Number);return s*60+(i||0)},W=()=>{const a=new Date;return`${String(a.getHours()).padStart(2,"0")}:${String(a.getMinutes()).padStart(2,"0")}`},C=()=>O("wiSmile")||null,S=()=>!!C()?.usuario,f=()=>O(B)||[],x=a=>X(B,a,48),j=a=>f().filter(s=>s.fecha===a),c=a=>{const s=d("#dia_sync");s.length&&(s[0].className=`dia_sync_dot dia_sync_${a}`)},Z=async(a=!1)=>{if(!a&&f().length)return c("ok");if(!S())return c("error");c("loading");try{const s=await H(L(z(I,F),R("usuario","==",C().usuario)));x(s.docs.map(i=>({...i.data(),_fsId:i.id,id:i.id}))),c("ok")}catch(s){console.error("❌ diario:",s),c("error")}},N=a=>{const s=f(),i=a._fsId||V(a.titulo),o={...a,_fsId:i,id:i},r=s.findIndex(_=>_._fsId===i);if(r>=0?s.splice(r,1,o):s.push(o),x(s),S()){c("saving");const _=C()||{},h={...o};delete h._fsId,Q(A(I,F,i),{...h,usuario:_.usuario||"",email:_.email||"",actualizado:U()},{merge:!0}).then(()=>c("ok")).catch(y=>{console.error("❌ upsert:",y),c("error")})}return o},aa=a=>{const s=a._fsId||a.id;x(f().filter(i=>i._fsId!==s)),S()&&(c("saving"),G(A(I,F,s)).then(()=>c("ok")).catch(i=>{console.error("❌ del:",i),c("error")}))},na=()=>`
<div class="dia_wrap">
...
</div>`,l=()=>{const a=j(e),s=d("#dia_events");s.find(".dia_event").remove(),a.forEach(i=>{const o=v[i.tipo]||v.otro,r=i.color||o.color,_=u(i.horaInicio||"09:00"),h=u(i.horaFin||"10:00"),y=E(_),q=Math.max(E(h-_),2.5),$=i.estado==="completado";s.append(`
      <div class="dia_event ${$?"dia_event_done":""}" data-id="${i._fsId}" 
        ...
        <div class="dia_event_content">
          ...
          <div class="dia_event_meta">
            ...
            <span class="dia_event_prio" style="color:${T[i.prioridad]||T.media}"><i class="fas fa-flag"></i></span>
          </div>
        </div>
