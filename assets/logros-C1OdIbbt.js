import{j as s}from"./vendor-gzd0YkcT.js";import{db as O}from"./firebase-D3LCa7rv.js";import{a as ao,q as lo,c as so,w as to,k as io,d as V,b as co,e as eo}from"./firebase-DYzFYKGm.js";import{e as no,d,h as E,N as m,j as $,g as Q,s as ro,f as K}from"./main-D1Fv2rJO.js";const T="wii_logros_v1",M="logros",_={personal:{label:"Personal",icon:"fa-user",color:"#FFB800"},trabajo:{label:"Trabajo",icon:"fa-briefcase",color:"#0EBEFF"},estudio:{label:"Estudio",icon:"fa-book",color:"#7000FF"},salud:{label:"Salud",icon:"fa-heart-pulse",color:"#FF5C69"},deporte:{label:"Deporte",icon:"fa-dumbbell",color:"#29C72E"},finanzas:{label:"Finanzas",icon:"fa-coins",color:"#00C9B1"},social:{label:"Social",icon:"fa-users",color:"#A855F7"},creativo:{label:"Creativo",icon:"fa-palette",color:"#F97316"},otro:{label:"Otro",icon:"fa-star",color:"#94A3B8"}},x=[{min:0,max:100,label:"Novato",icon:"fa-seedling",color:"#94A3B8",badge:"🌱"},{min:100,max:300,label:"Aprendiz",icon:"fa-bolt",color:"#29C72E",badge:"⚡"},{min:300,max:600,label:"Veterano",icon:"fa-shield",color:"#0EBEFF",badge:"🛡️"},{min:600,max:1e3,label:"Experto",icon:"fa-gem",color:"#7000FF",badge:"💎"},{min:1e3,max:2e3,label:"Maestro",icon:"fa-crown",color:"#FFB800",badge:"👑"},{min:2e3,max:9999,label:"Leyenda",icon:"fa-dragon",color:"#FF5C69",badge:"🔥"}],_o=[{id:"primer_logro",label:"Primer paso",icon:"fa-flag",color:"#29C72E",cond:a=>a.length>=1},{id:"x5",label:"Racha x5",icon:"fa-fire",color:"#FF5C69",cond:a=>a.length>=5},{id:"x10",label:"Décimo logro",icon:"fa-star",color:"#FFB800",cond:a=>a.length>=10},{id:"x25",label:"Imparable x25",icon:"fa-rocket",color:"#7000FF",cond:a=>a.length>=25},{id:"multcat",label:"Versátil",icon:"fa-shuffle",color:"#0EBEFF",cond:a=>new Set(a.map(o=>o.categoria)).size>=4},{id:"100xp",label:"100 XP",icon:"fa-bolt",color:"#29C72E",cond:(a,o)=>o>=100},{id:"500xp",label:"500 XP",icon:"fa-gem",color:"#7000FF",cond:(a,o)=>o>=500},{id:"1000xp",label:"Maestro XP",icon:"fa-crown",color:"#FFB800",cond:(a,o)=>o>=1e3}],y={personal:15,trabajo:20,estudio:25,salud:18,deporte:20,finanzas:15,social:12,creativo:18,otro:10},X=["#FFB800","#0EBEFF","#7000FF","#FF5C69","#29C72E","#A855F7","#00C9B1","#F97316","#94A3B8"],v=()=>new Date().toISOString().split("T")[0],S=a=>{const o=new Date;return o.setDate(o.getDate()+a),o.toISOString().split("T")[0]},go=a=>`${(a||"logro").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/\s+/g,"_").replace(/[^a-z0-9_]/g,"").slice(0,25)}_${Date.now()}`,j=a=>new Date(a+"T00:00:00").toLocaleDateString("es-PE",{day:"numeric",month:"short",year:"numeric"}),U=a=>{const o=Math.floor((new Date(v())-new Date(a+"T00:00:00"))/864e5);return o===0?"🟢 Hoy":o===1?"🔵 Ayer":o<7?`${o}d atrás`:o<30?`${Math.floor(o/7)}sem atrás`:`${Math.floor(o/30)}m atrás`},fo=a=>x.find(o=>a>=o.min&&a<o.max)||x[x.length-1],uo=a=>a.reduce((o,l)=>o+(y[l.categoria]||10)+(l.destacado?10:0),0),po=()=>[{label:"Hoy",fecha:v(),icon:"fa-circle-dot",color:"#0EBEFF"},{label:"Ayer",fecha:S(-1),icon:"fa-clock-rotate-left",color:"#FFB800"},{label:"Antier",fecha:S(-2),icon:"fa-backward",color:"#29C72E"},{label:"-1 semana",fecha:S(-7),icon:"fa-calendar-week",color:"#7000FF"},{label:"Este mes",fecha:new Date().toISOString().split("T")[0].slice(0,8)+"01",icon:"fa-flag",color:"#A855F7"}],z=()=>Q("wiSmile")||null,R=()=>!!z()?.usuario,g=()=>Q(T)||[],q=a=>ro(T,a,48),r=a=>{const o=s("#log_sync");o.length&&(o[0].className=`log_sync_dot log_sync_${a}`)},D=async(a=!1)=>{if(!a&&g().length)return r("ok");if(!R())return r("error");r("loading");try{const o=await ao(lo(so(O,M),to("usuario","==",z().usuario)));q(o.docs.map(l=>({...l.data(),_fsId:l.id,id:l.id}))),r("ok")}catch(o){console.error("❌ logros:",o),r("error")}},vo=a=>{const o=g(),l=a._fsId||go(a.titulo),t={...a,_fsId:l,id:l},c=o.findIndex(i=>i._fsId===l);if(c>=0?o.splice(c,1,t):o.push(t),q(o),R()){r("saving");const i=z()||{},u={...t};delete u._fsId,co(V(O,M,l),{...u,usuario:i.usuario||"",email:i.email||"",actualizado:eo()},{merge:!0}).then(()=>r("ok")).catch(b=>{console.error("❌ upsert:",b),r("error")})}return t},bo=a=>{const o=a._fsId||a.id;q(g().filter(l=>l._fsId!==o)),R()&&(r("saving"),io(V(O,M,o)).then(()=>r("ok")).catch(l=>{console.error("❌ del:",l),r("error")}))};let f=null,J=null,C="todos",L="",A="grid";const ho=()=>{let a=g().sort((o,l)=>(l.fecha||"").localeCompare(o.fecha||""));if(C!=="todos"&&(a=a.filter(o=>o.categoria===C||C==="destacado"&&o.destacado)),L){const o=L.toLowerCase();a=a.filter(l=>(l.titulo||"").toLowerCase().includes(o)||(l.descripcion||"").toLowerCase().includes(o))}return a},Eo=()=>`
<div class="log_wrap">
      ...
      <div class="log_field_row log_field_row2">
        ...
        <div class="log_field">
          ...
          <div class="log_colores" id="l_colores">
            ${X.map(a=>`<button type="button" class="log_color_opt" data-color="${a}" style="--c:${a}"></button>`).join("")}
          </div>
        </div>
      </div>
...
<div class="wiModal" id="modal_log_confirm">
  <div class="modalBody log_modal_confirm">
    ...
    </div>`);return}if(A==="timeline"){mo(a,o);return}a.forEach(l=>{const t=_[l.categoria]||_.otro,c=l.color||t.color,i=(y[l.categoria]||10)+(l.destacado?10:0);o.append(`
    <div class="log_card${l.destacado?" log_card_dest":""}" data-id="${l._fsId}" style="--lc:${c}">
      ...
      <div class="log_card_footer">
        <span class="log_card_fecha"><i class="fas fa-calendar"></i>${j(l.fecha||v())} <em>${U(l.fecha||v())}</em></span>
        <button class="log_card_edit" data-id="${l._fsId}" ${d("Editar")}><i class="fas fa-pen"></i></button>
      </div>
    </div>`)})},mo=(a,o)=>{const l={};a.forEach(c=>{const i=(c.fecha||v()).slice(0,7);l[i]||(l[i]=[]),l[i].push(c)});const t=s('<div class="log_timeline"></div>').appendTo(o);Object.entries(l).sort((c,i)=>i[0].localeCompare(c[0])).forEach(([c,i])=>{const[u,b]=c.split("-"),n=new Date(u,b-1,1).toLocaleDateString("es-PE",{month:"long",year:"numeric"});t.append(`<div class="log_tl_mes_lbl"><i class="fas fa-calendar-alt"></i> ${n} <span>${i.length} logro${i.length>1?"s":""}</span></div>`),i.forEach(e=>{const h=_[e.categoria]||_.otro,I=e.color||h.color,oo=(y[e.categoria]||10)+(e.destacado?10:0);t.append(`
      <div class="log_tl_item${e.destacado?" log_tl_dest":""}" data-id="${e._fsId}" style="--lc:${I}">
        <div class="log_tl_dot" style="background:${I}"><i class="fas ${e.destacado?"fa-star":h.icon}"></i></div>
        <div class="log_tl_body">
          ...
          <div class="log_tl_meta">
            ...
            <span class="log_tl_xp" style="color:${I}">+${oo} XP</span>
            <span class="log_tl_fecha"><i class="fas fa-clock"></i>${U(e.fecha||v())}</span>
          </div>
      ...
      </div>`)})})},W=a=>{s("#l_colores .log_color_opt").removeClass("active"),s(`#l_colores .log_color_opt[data-color="${a}"]`).addClass("active")},Y=()=>s("#l_colores .log_color_opt.active").data("color")||X[0],B=(a,o)=>{s("#log_m_hero").css("background",`linear-gradient(135deg,${a}dd,${a}88)`),s("#log_m_ico").css("background",a).html(`<i class="fas ${o}"></i>`)},Z=a=>{s("#l_destacado").html(`<i class="fas ${a?"fa-toggle-on":"fa-toggle-off"}"></i>`),s("#l_destacado_row").toggleClass("log_dest_on",!!a),s("#l_destacado").data("val",a?1:0)},w=()=>!!s("#l_destacado").data("val"),N=a=>{const o=po();s("#log_sugerencias").html(o.map(l=>`
      <button type="button" class="log_sug_btn${l.fecha===a?" active":""}"
        data-fecha="${l.fecha}" style="--sc:${l.color}" ${d(j(l.fecha))}>
        <i class="fas ${l.icon}"></i><span>${l.label}</span>
      </button>`).join(""))},P=()=>{const a=s("#l_cat").val()||"otro",o=w(),l=(y[a]||10)+(o?10:0);s("#log_m_xp").text(`+${l} XP`)},F=(a={})=>{f=a._fsId?a:null;const o=a.categoria||"personal",l=a.color||_[o]?.color||X[0],t=a.fecha||v();s("#l_titulo").val(a.titulo||""),s("#l_desc").val(a.descripcion||""),s("#l_fecha").val(t),s("#l_cat").val(o),W(l),Z(a.destacado||!1),B(l,_[o]?.icon||"fa-trophy"),N(t),P(),s("#log_m_tit").text(f?"Editar Logro":"Nuevo Logro"),s("#log_m_sub").text(f?"Modifica tu logro":"Registra tu éxito"),s("#l_eliminar").toggleClass("dpn",!f),K("modal_logro"),setTimeout(()=>s("#l_titulo").focus(),30)},G=()=>{const a=s("#l_titulo").val().trim();if(!a)return m("Título requerido","warning");const o=s("#l_fecha").val();if(!o)return m("Fecha requerida","warning");E("#l_guardar",!0,"Guardar");const l=s("#l_cat").val();vo({...f||{},titulo:a,descripcion:s("#l_desc").val().trim(),fecha:o,categoria:l,color:Y(),destacado:w(),creado:f?.creado||new Date().toISOString()}),$("modal_logro"),k(),p(),E("#l_guardar",!1,"Guardar");const t=(y[l]||10)+(w()?10:0);m(f?"Logro actualizado ✓":`🏆 +${t} XP · ¡Logro registrado!`,"success")},H=a=>{s("#log_confirm_nombre").text(a.titulo||"Sin título"),J=()=>{bo(a),$("modal_log_confirm"),k(),p(),m("Logro eliminado ✓","success")},K("modal_log_confirm")},$o=()=>{s(document).off(".log"),s(document).on("click.log","#log_nuevo,#log_empty_nuevo",()=>F()).on("click.log","#log_refresh",async()=>{E("#log_refresh",!0,""),localStorage.removeItem(T),await D(!0),k(),p(),E("#log_refresh",!1,""),m("Logros actualizados ✓","success")}).on("click.log",".log_fil",function(){C=s(this).data("fil"),s(".log_fil").removeClass("active"),s(this).addClass("active"),p()}).on("input.log","#log_buscar",function(){L=s(this).val(),p()}).on("click.log",".log_vista_btn",function(){A=s(this).data("vista"),s(".log_vista_btn").removeClass("active"),s(this).addClass("active"),p()}).on("click.log",".log_card",function(o){if(s(o.target).closest(".log_card_opts,.log_card_edit").length)return;const l=g().find(t=>t._fsId===s(this).data("id"));l&&F(l)}).on("click.log",".log_card_edit",function(o){o.stopPropagation();const l=g().find(t=>t._fsId===s(this).data("id"));l&&F(l)}).on("click.log",".log_card_opts",function(o){o.stopPropagation();const l=g().find(t=>t._fsId===s(this).data("id"));l&&H(l)}).on("click.log",".log_tl_item",function(o){if(s(o.target).closest(".log_card_edit,.log_card_opts").length)return;const l=g().find(t=>t._fsId===s(this).data("id"));l&&F(l)}).on("click.log",".log_sug_btn",function(){const o=s(this).data("fecha");s("#l_fecha").val(o),N(o),s("#log_m_sub").text(`Logrado: ${j(o)}`)}).on("change.log","#l_fecha",function(){N(s(this).val())}).on("change.log","#l_cat",function(){const o=s(this).val();B(Y(),_[o]?.icon||"fa-trophy"),P()}).on("click.log","#l_colores .log_color_opt",function(){W(s(this).data("color")),B(s(this).data("color"),_[s("#l_cat").val()]?.icon||"fa-trophy")}).on("click.log","#l_destacado",function(){Z(!w()),P()}).on("click.log","#l_cancelar",()=>$("modal_logro")).on("click.log","#l_guardar",G).on("keydown.log","#l_titulo",o=>{o.key==="Enter"&&G()}).on("click.log","#l_eliminar",()=>{f&&($("modal_logro"),H(f))}).on("click.log","#log_conf_no",()=>$("modal_log_confirm")).on("click.log","#log_conf_si",()=>J?.())},wo=async()=>{await D(),k(),p(),$o(),no(D,p),console.log("🏆 Logros v1.0 OK")},ko=()=>{s(document).off(".log"),console.log("🧹 Logros limpiado")};export{ko as cleanup,wo as init,Eo as render};
