import{j as s}from"./vendor-gzd0YkcT.js";import{db as x}from"./firebase-DqoUuv0g.js";import{g as V,q as X,c as J,w as W,s as Y,d as L,a as Z,b as aa}from"./firebase-CrRCUYpg.js";import{e as la,d as v,j as $,N as g,f as m,g as q,s as sa,h as N}from"./main-D7fYqA8Y.js";const P="wii_planes_v1",j="planes",f={personal:{label:"Personal",icon:"fa-user",color:"#FFB800"},trabajo:{label:"Trabajo",icon:"fa-briefcase",color:"#0EBEFF"},estudio:{label:"Estudio",icon:"fa-book",color:"#7000FF"},salud:{label:"Salud",icon:"fa-heart-pulse",color:"#FF5C69"},finanzas:{label:"Finanzas",icon:"fa-coins",color:"#29C72E"},viaje:{label:"Viaje",icon:"fa-plane",color:"#00C9B1"},proyecto:{label:"Proyecto",icon:"fa-diagram-project",color:"#A855F7"},otro:{label:"Otro",icon:"fa-circle",color:"#94A3B8"}},C={alta:{label:"Alta",color:"#FF5C69",icon:"fa-arrow-up"},media:{label:"Media",color:"#FFB800",icon:"fa-minus"},baja:{label:"Baja",color:"#29C72E",icon:"fa-arrow-down"}},y={activo:{label:"Activo",icon:"fa-circle-dot",color:"#0EBEFF"},pausado:{label:"Pausado",icon:"fa-pause-circle",color:"#FFB800"},completado:{label:"Completado",icon:"fa-circle-check",color:"#29C72E"},cancelado:{label:"Cancelado",icon:"fa-circle-xmark",color:"#94A3B8"}},I=["#0EBEFF","#29C72E","#7000FF","#FF5C69","#FFB800","#00C9B1","#A855F7","#94A3B8"],A=()=>new Date().toISOString().split("T")[0],na=l=>`${(l||"plan").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/\s+/g,"_").replace(/[^a-z0-9_]/g,"").slice(0,25)}_${Date.now()}`,T=l=>l?new Date(l+"T00:00:00").toLocaleDateString("es-PE",{day:"numeric",month:"short",year:"numeric"}):"—",oa=l=>{if(!l)return"";const a=Math.ceil((new Date(l+"T00:00:00")-new Date(A()+"T00:00:00"))/864e5);return a<0?`<span class="pln_vencido">${-a}d atrás</span>`:a===0?'<span class="pln_hoy">Hoy</span>':`<span class="pln_dias">${a}d</span>`},ta=l=>{const a=l.pasos?.length||0,o=l.pasos?.filter(n=>n.done).length||0;return a?Math.round(o/a*100):l.estado==="completado"?100:0},B=()=>q("wiSmile")||null,D=()=>!!B()?.usuario,d=()=>q(P)||[],S=l=>sa(P,l,48),c=l=>{const a=s("#pln_sync");a.length&&(a[0].className=`pln_sync_dot pln_sync_${l}`)},w=async(l=!1)=>{if(!l&&d().length)return c("ok");if(!D())return c("error");c("loading");try{const a=await V(X(J(x,j),W("usuario","==",B().usuario)));S(a.docs.map(o=>({...o.data(),_fsId:o.id,id:o.id}))),c("ok")}catch(a){console.error("❌ planes:",a),c("error")}},R=l=>{const a=d(),o=l._fsId||na(l.titulo),n={...l,_fsId:o,id:o},t=a.findIndex(i=>i._fsId===o);if(t>=0?a.splice(t,1,n):a.push(n),S(a),D()){c("saving");const i=B()||{},u={...n};delete u._fsId,Y(L(x,j,o),{...u,usuario:i.usuario||"",email:i.email||"",actualizado:aa()},{merge:!0}).then(()=>c("ok")).catch(b=>{console.error("❌ upsert:",b),c("error")})}return n},ia=l=>{const a=l._fsId||l.id;S(d().filter(o=>o._fsId!==a)),D()&&(c("saving"),Z(L(x,j,a)).then(()=>c("ok")).catch(o=>{console.error("❌ del:",o),c("error")}))};let h="todos",F="",e=null,G=null,H="grid";const ca=()=>{let l=d();if(h!=="todos"&&(l=l.filter(a=>a.estado===h||a.categoria===h)),F){const a=F.toLowerCase();l=l.filter(o=>(o.titulo||"").toLowerCase().includes(a)||(o.descripcion||"").toLowerCase().includes(a))}return l.sort((a,o)=>{const n={alta:0,media:1,baja:2};return(n[a.prioridad]||1)-(n[o.prioridad]||1)})},ba=()=>`
<div class="pln_wrap">
