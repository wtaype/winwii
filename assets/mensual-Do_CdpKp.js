...
</div>`,_=()=>{const s=ss(l,n),a=es(l,n),i=os(l,n),o=e("#mes_days").empty();for(let t=0;t<a;t++)o.append('<div class="mes_day mes_day_empty"></div>');for(let t=1;t<=s;t++){const f=as(t,l,n),v=i.filter(A=>A.fecha===f),B=K(f),T=W(f),z=f===d;let M="";v.length&&(M=`<div class="mes_day_dots">${[...new Set(v.slice(0,3).map(k=>k.color||g[k.tipo]?.color||"#94A3B8"))].map(k=>`<span class="mes_dot" style="background:${k}"></span>`).join("")}${v.length>3?`<span class="mes_dot_more">+${v.length-3}</span>`:""}</div>`),o.append(`
      <div class="mes_day ${B?"mes_day_hoy":""} ${T?"mes_day_past":""} ${z?"mes_day_sel":""}" data-fecha="${f}">
