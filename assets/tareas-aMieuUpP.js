import{j as o}from"./vendor-gzd0YkcT.js";import{S as Z}from"./sortable.esm-BzJVkfZx.js";import{db as I}from"./firebase-BS8Opw9z.js";import{g as aa,q as ta,c as sa,w as oa,s as ia,f as H,i as ea,h as ra}from"./firebase-CiLZDPlJ.js";import{k as la,c as b,q as m,g as V,s as ca,j as p,t as X,o as w}from"./main-BEFDoIKE.js";const j="wii_tareas_v1",O="tareas",u={pendiente:"Pendiente",progreso:"En progreso",revision:"Revisión",hecho:"Hecho"},S={pendiente:"fa-circle-dot",progreso:"fa-spinner",revision:"fa-eye",hecho:"fa-circle-check"},y={pendiente:"#FFB800",progreso:"#0EBEFF",revision:"#7000FF",hecho:"#29C72E"},K={pendiente:"progreso",progreso:"revision",revision:"hecho",hecho:"hecho"},Q={pendiente:"pendiente",progreso:"pendiente",revision:"progreso",hecho:"revision"},v={trabajo:{label:"Trabajo",icon:"fa-briefcase",color:"#29C72E"},estudio:{label:"Estudio",icon:"fa-book",color:"#7000FF"},web:{label:"Web",icon:"fa-globe",color:"#0EBEFF"},personal:{label:"Personal",icon:"fa-user",color:"#FFB800"},otros:{label:"Otros",icon:"fa-circle",color:"#94A3B8"}},na={alta:"#FF5C69",media:"#FFB800",baja:"#29C72E"},N={alta:0,media:1,baja:2},k=["#29C72E","#0EBEFF","#7000FF","#FF5C69","#FFB800","#94A3B8"],da=()=>new Date().toISOString().split("T")[0],_a=a=>a?new Date(a+"T00:00:00").toLocaleDateString("es-PE",{day:"numeric",month:"short"}):"",ua=a=>{if(!a)return"";const t=Math.ceil((new Date(a+"T00:00:00")-new Date(da()+"T00:00:00"))/864e5);return t<0?`<span class="tar_vencido">Vencido ${-t}d</span>`:t===0?'<span class="tar_hoy_tag">Hoy</span>':`${t}d`},q=()=>{const a=new Date;return a.setDate(a.getDate()+7),a.toISOString().split("T")[0]},fa=a=>`${(a||"tarea").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/\s+/g,"_").replace(/[^a-z0-9_]/g,"").slice(0,25)}_${Date.now()}`,D=()=>V("wiSmile")||null,B=()=>!!D()?.usuario,f=()=>V(j)||[],P=a=>ca(j,a,48),c=a=>{const t=o("#tar_sync_dot");if(!t.length)return;t[0].className=`tar_sync_dot tar_sync_${a}`;const s={loading:"Cargando…",ok:"Sincronizado",error:"Sin conexión",saving:"Guardando…"};t.attr("data-witip",s[a]||"")},pa=a=>{const t=D()||{},s={...a};return delete s._fsId,{...s,usuario:t.usuario||"",email:t.email||"",actualizado:ra()}},va=(a,t)=>({...t,_fsId:a,id:a}),F=async(a=!1)=>{if(!a&&f().length)return c("ok");if(!B())return c("error");c("loading");try{const t=await aa(ta(sa(I,O),oa("usuario","==",D().usuario)));P(t.docs.map(s=>va(s.id,s.data()))),c("ok")}catch(t){console.error("❌ tareas:",t),c("error")}},E=a=>{const t=f(),s=a._fsId||fa(a.titulo),e={...a,_fsId:s,id:s},r=t.findIndex(i=>i._fsId===s);return r>=0?t.splice(r,1,e):t.push(e),P(t),B()&&(c("saving"),ia(H(I,O,s),pa(e),{merge:!0}).then(()=>c("ok")).catch(i=>{console.error("❌ upsert:",i),c("error")})),e},ba=a=>{const t=a._fsId||a.id;P(f().filter(s=>s._fsId!==t)),B()&&(c("saving"),ea(H(I,O,t)).then(()=>c("ok")).catch(s=>{console.error("❌ del:",s),c("error")}))},ha=async()=>{w("#tar_refresh",!0,""),localStorage.removeItem(j),await F(!0),d(),w("#tar_refresh",!1,""),p("Tareas actualizadas ✓","success")};let C="todos",x="",n=null,U=null,g=[];const ma=()=>{let a=f();if(C!=="todos"&&(a=a.filter(t=>t.estado===C)),x){const t=x.toLowerCase();a=a.filter(s=>(s.titulo||"").toLowerCase().includes(t))}return a},ga=()=>{const a=o("#tar_quick_input").val().trim();if(!a)return p("Escribe un título","warning");const t=o("#tar_quick_tipo").val()||"trabajo";E({titulo:a,fecha:q(),prio:"media",tipo:t,estado:"pendiente",color:v[t]?.color||k[0],subtareas:[],historial:[],creado:new Date().toISOString()}),d(),o("#tar_quick_input").val("").focus(),p("Tarea creada ✓","success")},$a=(a,t)=>{const s=f().find(l=>l._fsId===a);if(!s)return;const e=t==="next"?K[s.estado||"pendiente"]:Q[s.estado||"pendiente"];if(e===s.estado)return;const r=s.historial||[];r.push({de:s.estado,a:e,fecha:new Date().toISOString()}),E({...s,estado:e,historial:r,completado:e==="hecho"?new Date().toISOString():""}),d();const i={pendiente:"⏳",progreso:"🔄",revision:"👀",hecho:"✅"}[e];p(`${i} ${s.titulo} → ${u[e]},'success'`)},Ia=()=>`
<div class="tar_wrap">
